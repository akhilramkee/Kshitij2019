...
float normalize(int channel, float inp)
{
  switch(channel)
  {
    ...
    case 3:
       angleabs = (((inp - 987)/(float)(1989-987)*100.0*1.8));
       if(angleabs<0){
         angleabs = 0;
...
void driveCrane()
{
  ...
  {
    for(pos_s1 = prevAngleA, pos_s2 = prevAngleB; pos_s1 >= currentAngleA; pos_s1 -= 1.8, pos_s2 -= 1.8)
    {
      servoA.write( pos_s1 );
      servoB.write( pos_s2);
      delay( 25 );
...
void loop() {
   ...
   rpm_calc();
   if(abs(prevAngleA-currentAngleA)>=10)
   {
     driveCrane();
   }
